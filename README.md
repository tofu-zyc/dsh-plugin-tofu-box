...
| --- | --- | --- | --- | --- |
| **image-generation** | 独立绘图模型配置、Images API 工具和直接绘图面板，预览与原图下载；[使用说明](dsh-plugin-image-generation/README.md) | ≥ 0.1.6-alpha.1，实测 0.1.6-alpha.2 | Win11 实测；Linux/macOS 未测 | `dsh plugin --profile web add git+https://github.com/tofu-zyc/dsh-plugin-tofu-box.git#path:dsh-plugin-image-generation` |
| **model-search** | 模型选择菜单加一个搜索框，按名称 / ID / 描述 / provider 过滤 | 0.1.5-rc ~ 0.1.6-alpha.2 实测 | Win11 实测；Linux/macOS 未测 | `dsh plugin --profile web add git+https://github.com/tofu-zyc/dsh-plugin-tofu-box.git#path:dsh-plugin-model-search` |
| **model-tuning** | 集中配置各模型的推理档位、上下文窗口、最大输出、多模态支持等 | 0.1.5-rc ~ 0.1.6-alpha.2 实测 | Win11 实测；Linux/macOS 未测 | `dsh plugin --profile web add git+https://github.com/tofu-zyc/dsh-plugin-tofu-box.git#path:dsh-plugin-model-tuning` |
| **read-image-preview** | 工具结果里的图片（read_image、截图等）渲染成图片卡，点击放大、滚轮缩放、拖动平移 | 0.1.5-rc ~ 0.1.6-alpha.2 实测 | Win11 实测；Linux/macOS 未测 | `dsh plugin --profile web add git+https://github.com/tofu-zyc/dsh-plugin-tofu-box.git#path:dsh-plugin-read-image-preview` |
...
| --- | --- | --- | --- | --- |
| **image-generation** | Separate image models, Images API tools, a direct generation panel, previews and original downloads ([README](dsh-plugin-image-generation/README.md)) | ≥ 0.1.6-alpha.1, tested on 0.1.6-alpha.2 | Tested on Win11; Linux/macOS untested | `dsh plugin --profile web add git+https://github.com/tofu-zyc/dsh-plugin-tofu-box.git#path:dsh-plugin-image-generation` |
| **model-search** | Search box in the model picker (name / ID / description / provider) | Tested on 0.1.5-rc ~ 0.1.6-alpha.2 | Tested on Win11; Linux/macOS untested | `dsh plugin --profile web add git+https://github.com/tofu-zyc/dsh-plugin-tofu-box.git#path:dsh-plugin-model-search` |
| **model-tuning** | Per-model reasoning effort, context window, max output and multimodal support in one settings page | Tested on 0.1.5-rc ~ 0.1.6-alpha.2 | Tested on Win11; Linux/macOS untested | `dsh plugin --profile web add git+https://github.com/tofu-zyc/dsh-plugin-tofu-box.git#path:dsh-plugin-model-tuning` |
| **read-image-preview** | Renders images in tool results as zoomable cards (click to enlarge, wheel to zoom, drag to pan) | Tested on 0.1.5-rc ~ 0.1.6-alpha.2 | Tested on Win11; Linux/macOS untested | `dsh plugin --profile web add git+https://github.com/tofu-zyc/dsh-plugin-tofu-box.git#path:dsh-plugin-read-image-preview` |
